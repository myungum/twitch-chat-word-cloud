...
```
띄어쓰기가 비교적 잘 지켜졌음에도 <span style="color:#dd5555">신조어를 잘 분리해 내지 못합니다.</span>
...
```
기대보다 좋지 않은 성능을 보여줍니다. 그래도 대상만 잘 맞으면 좋은 성능을 낼 것 같습니다. 트위치가 게임 방송 플랫폼인 만큼 <span style="color:#dd5555">게임 용어는 Okt보다 긍정적인 면이 있습니다.</span>
```
...
|--|--|--|--|
|<b>신조어 비율</b>|8%|8%|1%|
|<b>예시</b>|롤, 딜, 겜, 호우, 탑, 플, 컷, 징끼|ㅋㅋ, ㄷㄷ, ㄹㅇ, ㅠㅠ, ㅔ,ㄱㅇㅇ, ㄱㄱ, 걍|걍
|<b>특징</b>|한 글자 외국어가 잘 검출됩니다.|자음 신조어가 잘 검출됩니다. 트위치 이모티콘이 단어로 검출되어서 전처리 또는 예외처리가 필요합니다.|'아니', '진짜' 등 온전하지만 자주 쓰이는 단어가 잘 검출됩니다. 신조어가 잘 검출되지 않습니다. 메모리 부족으로 많은 데이터를 읽을 수 없습니다.|

<br>

## 핵심 키워드 찾기
신조어 분석을 어느 정도 할 수 있게 되었으니, 간단하게 활용해 보기 위해 하루의 핵심 키워드를 뽑아봅니다. <br>
단어의 빈도만 따지기에는 조사, 감탄사같이 <span style="color:#dd5555"> 의미없는 일반적인 단어의 빈도가 더 많습니다.</span> <br>
따라서 이를 분석할 별도의 알고리즘이 필요합니다. <br><br>
다음과 같이 적당한 식으로 점수를 냅니다.<br>
$$(점수) = (증가율) * \log_{2}(실제 빈도)$$
$$(증가율) = \frac{(실제 빈도) - (기대 빈도)}{(기대 빈도)}$$
$$(기대 빈도) = MAX(일주일 평균 빈도, 어제 빈도)$$
<br>
<span style="color:#117711">결과 : 김이나, 제페토, 무례맨, 루미네, 스빠룬키, 심연공주, 근든링, 심술맨, 타탈, 독퍼, 나히다, 침스라이팅, 워존...</span><br><br>
